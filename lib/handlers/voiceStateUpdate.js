...
const handleVoiceStateUpdate = async (manager, oldState, newState) => {
    const voiceChannelLeft = !!oldState.channelId && !newState.channelId;
    const voiceChannelMoved = !!oldState.channelId &&
        !!newState.channelId &&
    ...
    if (voiceChannelLeft || voiceChannelMoved) {
        ...
        if (parentChannel) {
            ...
            if ((parentChannel.options.childAutoDeleteIfEmpty &&
                ...
                childToDelete.channel
                    ...
                    parentChannel.children = parentChannel.children.filter(child => child.channel.id !== childToDelete.channel.id);
                    manager.emit("childDelete", newState.member, childToDelete.channel, manager.client.channels.cache.get(parentChannel.channelID));
                })
                    .catch(error => {
